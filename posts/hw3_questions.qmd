...
for patent_axis, patent_status in zip(axes, patent_histogram_statuses):
    ...
    patent_axis.set_title(patent_status)
    patent_axis.set_xlabel("Patents awarded in last 5 years")
    patent_axis.set_ylabel("Number of firms")
...
region_bar_width = 0.35

fig, region_axis = plt.subplots(figsize=(10, 4))
...
region_axis.set_xticklabels(region_distribution_order, rotation=30, ha="right")
region_axis.set_ylabel("Share of firms (%)")
region_axis.set_xlabel("Region")
region_axis.set_title("Regional distribution by customer status")
region_axis.legend()
...
fig, age_axes = plt.subplots(1, 2, figsize=(11, 4), sharex=True, sharey=True)
...
for age_axis, age_status in zip(age_axes, age_histogram_statuses):
    ...
    age_axis.set_title(age_status)
    age_axis.set_xlabel("Firm age")
    age_axis.set_ylabel("Number of firms")
...
To check that my manual maximum likelihood estimates are correct, I next estimate the same model using `statsmodels`' built-in Poisson GLM function.
